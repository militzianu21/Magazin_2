...
Public Class Form_afiseaza_nir
    ...
    Private Sub Form_afiseaza_nir_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If nir_TB.Text = Nothing Then
            nir = ""
            nir_TB.Enabled = True
        Else : nir = "AND marfa.nir=" & nir_TB.Text
        ...
        Using dbconn As New MySqlConnection("Data Source=localhost;user id=root;password=*******;port=3306;database=magazin")
            ...
            Dim sql_tot As String = "SELECT marfa.id,marfa.produs,marfa.bucati,marfa.pret,marfa.pret_intrare,marfa.data,niruri.tva,niruri.nr_factura,niruri.cif_firma,niruri.nume_firma " _
                                    ...
                                    & "WHERE marfa.data=@data " & nir & " ORDER BY id asc"
        ...
        If bun = True Then
            ...
            If row < DataGridView1.Rows.Count Then
                ...
                For j = 0 To DataGridView1.Columns.Count - 1
                    DataGridView1.Rows(row).Cells(j).Style.BackColor = Color.LightPink
                Next
                Dim crt As String = DataGridView1.Rows(row).Cells("crt").Value.ToString
                Dim produs_ As String = DataGridView1.Rows(row).Cells("produs").Value.ToString
                Dim buc_ As String = DataGridView1.Rows(row).Cells("buc").Value.ToString
                Dim cant_ As String = DataGridView1.Rows(row).Cells("cant").Value.ToString
                Dim pr_ach_ As String = DataGridView1.Rows(row).Cells("pret_ach").Value.ToString
                Dim pr_ach_tva_ As String = DataGridView1.Rows(row).Cells("pret_ach_tva").Value.ToString
                Dim pret_ As String = DataGridView1.Rows(row).Cells("pret").Value.ToString
                Dim valoare_ As String = DataGridView1.Rows(row).Cells("valoare").Value.ToString
    ...
    Private Sub Save_Bu_Click(sender As Object, e As EventArgs) Handles save_Bu.Click
        ...
        Else
            ...
            For i As Integer = 0 To DataGridView1.Rows.Count - 1
                ...
                Dim sql_prod As String = "UPDATE marfa SET produs=@produs,pret=@pret,bucati=@bucati,nir=@nir,data=@data,pret_intrare=@pret_intrare,magazin=@magazin WHERE id=@id"
                Try
            ...
            Dim sql_ins As String = "INSERT INTO niruri (nir,data,valoare,tva,nr_factura,nume_firma,cif_firma,magazin) " _
    ...
    Private Sub PrintNir_BU_Click(sender As Object, e As EventArgs) Handles PrintNir_BU.Click
        ...
        tf.DrawString(data_nir, top_font, XBrushes.Black, _
        ...
        graph.DrawString(subsemnatii, text_font, XBrushes.Black, _
                         New XRect(45, 67, 560, pdfPage.Height.Point), XStringFormats.TopLeft)

        graph.DrawString(firma & " " & f_jur, top_font, XBrushes.Black, _
                         New XRect(560, 67, 150, pdfPage.Height.Point), XStringFormats.TopLeft)

        graph.DrawString(auto, text_font, XBrushes.Black, _
        ...
        For i = 0 To DGV.Rows.Count - 1
            ...
            graph.DrawString(produs, text_font, XBrushes.Black, _
                             New XRect(75, nxtRow, 190, pdfPage.Height.Point), XStringFormats.TopCenter)

            'graph.DrawLine(pen, 265, 100, 265, 132) ' V

            graph.DrawString(buc, text_font, XBrushes.Black, _
                             New XRect(265, nxtRow, 30, pdfPage.Height.Point), XStringFormats.TopCenter)

            'graph.DrawLine(pen, 295, 100, 295, 132) ' V

            graph.DrawString(cant, text_font, XBrushes.Black, _
                             New XRect(295, nxtRow, 100, pdfPage.Height.Point), XStringFormats.TopCenter)

            'graph.DrawLine(pen, 395, 100, 395, 132) ' V

            tf.DrawString(Format(pret_intrare, "#,#0.00"), text_font, XBrushes.Black, _
                             New XRect(395, nxtRow, 95, pdfPage.Height.Point), XStringFormats.TopLeft)

            'graph.DrawLine(pen, 495, 100, 495, 132) ' V

            tf.DrawString(Format(valoare_intrare, "#,#0.00"), text_font, XBrushes.Black, _
                             New XRect(495, nxtRow, 95, pdfPage.Height.Point), XStringFormats.TopLeft)

            'graph.DrawLine(pen, 595, 100, 595, 132) ' V

            tf.DrawString(Format(pret_vanzare, "#,#0.00"), text_font, XBrushes.Black, _
                             New XRect(595, nxtRow, 95, pdfPage.Height.Point), XStringFormats.TopLeft)

            'graph.DrawLine(pen, 695, 100, 695, 132) ' V

            tf.DrawString(Format(valoare_vanzare, "#,#0.00"), text_font, XBrushes.Black, _
        ...
        graph.DrawString("Membrii comisiei (Nume, Prenume, Semnatura)", mic_font, XBrushes.Black, _
